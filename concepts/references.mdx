...
Type `@` in a draft to open the reference picker. Choose either a branch or a specific message. NodePad inserts the visible token into the draft and stores the actual linked node behind it. That matters because the text token alone is not the source of truth.
...
That is useful for comparing recommendations, combining research findings, rewriting a draft using evidence gathered elsewhere, or deciding between alternate approaches.
...
<NavList items={[
  { title: 'Merging', body: 'How a merge composer assembles source messages.', href: '/concepts/merging' },
  { title: 'Forking', body: 'Branching is what makes cross-branch references possible.', href: '/concepts/forking' },
]} />
