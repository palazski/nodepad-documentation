...
<Note>
  Your internal registry (Nexus / Harbor / Artifactory) must proxy GHCR (`ghcr.io`) and cache the NodePad images.
</Note>

Point the chart at your mirror via a single value:
...
<Steps>
  <Step title="Download or unzip the bundle">
    ```bash
  ...
  <Step title="Load images (offline) or let compose pull from GHCR">
    ```bash
...
<Card title="Verify the install" icon="circle-check" type="check">
  ```bash
  # Pods are Running
...
<Columns cols={2}>
  <Card title="Upgrade & rollback playbook" icon="circle-arrow-up" href="/enterprise/operations/upgrade">
    How to upgrade safely, roll back, and handle migration-incompatible rollbacks.
  </Card>
  <Card title="Air-gapped install details" icon="shield" href="/enterprise/operations/air-gapped">
