...
<Checkbox>Read the [changelog](/changelog) for the target version. Note any breaking changes.</Checkbox>
<Checkbox>Back up the database (see below).</Checkbox>
<Checkbox>For Kubernetes: verify the pre-upgrade hook succeeded before api pods roll.</Checkbox>
<Checkbox>Smoke-test after: `/api/health`, admin login, canvas load, LLM stream.</Checkbox>
<Checkbox>Monitor logs and error rates for 15 minutes post-rollout.</Checkbox>
