...
<Steps>
  ...
  <Step title="Verify the checksum (sha256) against the release notes">
    Release notes list the expected hash.
  </Step>
  <Step title="Move the zip through your approved transfer path">
...
- **No automatic update notifications** — operators must track releases manually via the [changelog](/changelog) or GitHub Releases.
- **No Sentry/PostHog telemetry** — both are opt-in and can be left blank.
- **No in-app version check.** To see what's running:
  ```bash
